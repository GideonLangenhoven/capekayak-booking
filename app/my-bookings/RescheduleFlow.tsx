  ...
  if (rebookConfirmSlot) {
    ...
    return (
      <div className="app-container max-w-lg page-wrap py-8">
              ...
              {isClaim && rescheduling.qty > 1 ? (
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => setRescheduleQty(Math.max(1, rescheduleQty - 1))} disabled={rescheduleQty <= 1}
                    className="h-7 w-7 rounded-full border border-[color:var(--border)] text-[color:var(--text)] disabled:opacity-30 hover:border-[color:var(--accent)] transition-colors" aria-label="Fewer guests">−</button>
                  ...
                  <button type="button" onClick={() => setRescheduleQty(Math.min(rescheduling.qty, rescheduleQty + 1))} disabled={rescheduleQty >= rescheduling.qty}
                    className="h-7 w-7 rounded-full border border-[color:var(--border)] text-[color:var(--text)] disabled:opacity-30 hover:border-[color:var(--accent)] transition-colors" aria-label="More guests">+</button>
                </div>
              ) : (
                <p className="font-medium text-[color:var(--text)]">{qty} {qty === 1 ? "person" : "people"}</p>
              )}
              {isClaim && rescheduling.qty > 1 && <p className="mt-1 text-[11px] text-[color:var(--textMuted)]">Fewer guests? The difference is refunded.</p>}
            </div>
        ...
        {notEnoughSpots && (
          <p className="mb-3 text-sm text-red-600">Only {slotSpots} {slotSpots === 1 ? "spot" : "spots"} left on this slot — reduce the guest count or pick another date.</p>
        )}
